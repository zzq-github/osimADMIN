<template>
  <div class="mod-user">
    ...
    <!-- 添加,编辑弹窗 -->
    <el-dialog title="批量换新" :visible.sync="addVisible" width="800px" :close-on-click-modal="false">
      <el-table
        :data="dataList"
        border
        height="480px"
        ...
        <el-table-column prop="oldSerialNumber" header-align="center" align="center" label="旧产品序列号"></el-table-column>
        <el-table-column prop="newSerialNumber" header-align="center" align="center" label="新产品序列号"></el-table-column>
      </el-table>
...
<script>
  import { uploadExcleExchange, unbindListExchange } from '@/api/baseApi'
  ...
  export default {
    ...
    methods: {
      ...
      doUpload (data) {
        ...
        this.$http.post(uploadExcleExchange, formdata).then(res => {
      ...
      submitAddForm (formName) {
        this.$confirm(`是否确认提交?`, '提示', {type: 'warning'}).then(() => {
          ...
          // 新增提交
          this.$http.post(unbindListExchange, this.dataList).then(res => {
            if (res.data.code === '200') {
              this.$notify({title: '提示', message: `成功换新${res.data.data}条！`, type: 'success'})
